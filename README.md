...
│               └── scripts/                # export-raster.mjs — SVG-to-PNG/ICO icon exporter
└── docs/
...
- **For logo-design:** No hard requirements — the drawing and the arithmetic need only built-in tools. Playwright MCP is optional but strongly recommended: without it every render-dependent check is recorded `UNRUN` rather than passed, and a review caps at `PARTIAL — evidence-limited`. A local SVG rasteriser (`resvg`, `rsvg-convert`, Inkscape, or ImageMagick) is optional too: without one the raster icon set is recorded `UNRUN` rather than failed, and the SVG set still ships. `docs/design/MASTER.md` from `ui-design-system` supplies palette and typography when present; the skill ships in `currentColor` when it is absent.
